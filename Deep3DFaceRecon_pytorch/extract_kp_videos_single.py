...
class KeypointExtractor():
    ...
    def extract_keypoint(self, images, name=None):
        if isinstance(images, list):
            ...
            np.savetxt(name, keypoints.reshape(-1))
            return keypoints
        else:
            while True:
                try:
...
def run(data):
    ...
    name = filename.replace(opt.input_dir,opt.output_dir)
    name=name.replace('.mp4','.txt')
...
if __name__ == '__main__':
    ...
    filenames = list()
    filenames = sorted(glob.glob(f'{opt.input_dir}/*.png', recursive=True))
