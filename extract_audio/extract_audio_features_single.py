...
def process_audio_files(root_path, model_name, window_size):
    ...
    #audio_files = [audio_files[0],audio_files[0]]
    ...
    for audio_file in tqdm(audio_files, desc=f"Processing audio files with {model_name}"):
        out_name = audio_file.replace('.wav', '.npy')#.replace('vox-celeb-wav', 'vox-celeb-audemb')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataroot', type=str, default='input_single/*.wav', help='Root path for audio files')
